    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            for d in range(D):
                ...
                combo = stack([X[:, d], Y], axis=1)
                lFeats = combo[combo[:, 0] < 0.5]
                leftY  = util.mode(lFeats[:,1])    ### TODO: YOUR CODE HERE

                rFeats = combo[combo[:, 0] >= 0.5]
                rightY = util.mode(rFeats[:, 1])    ### TODO: YOUR CODE HERE
                ...
                ### TODO: YOUR CODE HERE
                lerr = len(lFeats[lFeats[:, 1] != leftY])
                rerr = len(rFeats[rFeats[:, 1] != rightY])
                error = lerr + rerr
            ...
            if bestFeature < 0:
                ...
                ### TODO: YOUR CODE HERE
                lFeat = X[:, bestFeature] < 0.5
                rFeat = invert(lFeat)
                used.append(bestFeature)
                self.left.trainDT(X[lFeat], Y[lFeat], maxDepth - 1, used)
                self.right.trainDT(X[rFeat, :], Y[rFeat], maxDepth - 1, used)

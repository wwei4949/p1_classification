...
class FirstFeatureClassifier(BinaryClassifier):
    ...
    def predict(self, X):
        ...
        ### TODO: YOUR CODE HERE
        if (X[0] > 0):
            return self.predClass[1]

            return self.predClass[0]
    ...
    def train(self, X, Y):
        ...
        ### TODO: YOUR CODE HERE
        combined = list(zip(X[:, 0], Y))  # np.array([X,Y])
        greatr = [b for (a, b) in combined if a > 0]
        lesseq = [b for (a, b) in combined if a <= 0]

        self.predClass = {1: util.mode(greatr), 0: util.mode(lesseq)}

...
class KNN(BinaryClassifier):
    ...
    def predict(self, X):
        ...
        if self.trY.size == 0:
            ...
            ### TODO: YOUR CODE HERE
            tridx = range(0, len(self.trX))
            trDist = []

            for ex in tridx:
             tmp_dist = linalg.norm(X - self.trX[ex])
            trDist.append(tmp_dist)

            s_dist = sorted(zip(trDist, tridx))

            for (dist, idx) in s_dist[:K]:
             val = val + self.trY[idx]
